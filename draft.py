...
st.markdown(
    f"""
    <div style="position: fixed; top: 0px; left: 0; right: 0; display: flex; align-items: center; justify-content: center; height: 80px; z-index: 100000;">
    ...
    st.markdown(
        ...
        function sendMessage(payload) {
            ...
            if (user_input) {
                ...
                var user_message_html = '<div class="chat-message user"><div class="chat-bubble" style="background-color: #DCF8C6;">' + user_input + '</div></div>';
                chat_container.innerHTML += user_message_html;
                document.getElementById("user_input").value = "";
                
                // Send user input to backend
                const Http = new XMLHttpRequest();
                const url='http://kauza.serveo.net/webhooks/rest/webhook/""" + tenant_id + """/agents/""" + agent_id + """';
                Http.open("POST", url);
                Http.setRequestHeader("Content-Type", "application/json");
                Http.send(JSON.stringify({"sender": "''' + session_id + '''", "message": user_input}));

                Http.onreadystatechange = (e) => {
                    if (Http.readyState === 4 && Http.status === 200) {
                        const response = JSON.parse(Http.responseText);
                        for (let i = 0; i < response.length; i++) {
                            displayResponse(response[i]);
                        }
                    }
                };
            }
